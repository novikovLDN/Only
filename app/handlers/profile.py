...
async def show_profile(message: Message, user, t, session=None) -> None:
    ...
    sub_until = user.subscription_until.strftime("%d.%m.%Y") if user.subscription_until else t("no_subscription")
    text = f"{t('profile')}\n{t('registration_date', date=reg_date)}\n{t('invited_count', count=count)}\n{t('subscription_until', date=sub_until)}"
